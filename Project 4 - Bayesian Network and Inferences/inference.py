...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict = {}
    allTupleCombination = []
    # using set and then converting to list to avoid duplicate manhattan distance in the domain
    allManhattanDistanceCombinationSet = set()
    
    for x in range(0, X_RANGE):
        for y in range(0, Y_RANGE):
            allTupleCombination.append((x,y))
            # this eventually generates all non-negative manhattan distance between pacman and ghost
            allManhattanDistanceCombinationSet.add(x+y+MAX_NOISE)
            if x+y-MAX_NOISE >= 0:
                allManhattanDistanceCombinationSet.add(x+y-MAX_NOISE)
    
    variableDomainsDict[GHOST0] = allTupleCombination
    variableDomainsDict[GHOST1] = allTupleCombination
    variableDomainsDict[PAC] = allTupleCombination
    variableDomainsDict[OBS0] = list(allManhattanDistanceCombinationSet)
    variableDomainsDict[OBS1] = list(allManhattanDistanceCombinationSet)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # this provides every factors based on given evidenceDict
        allFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # for every elimination variable (hidden variable), perform joining (summation)
        # unjoinedFactors -> factors that are outside of summation
        # joinedFactors -> factors that are inside summation that should be eliminated soon
        for ev in eliminationOrder:
            unjoinedFactors, joinedFactors = joinFactorsByVariable(allFactors, ev)
            allFactors = unjoinedFactors
            
            # simple length check to avoid edge case mentioned in the problem description
            if len(joinedFactors.unconditionedVariables()) > 1:
                # from joinedFactors, eliminate elimination variable and append to allFactors
                # allFactors at this point is unjoinedFactors that are outside of summation
                allFactors.append(eliminate(joinedFactors, ev))

        # allFactors at this point is simply list of Factors that should be joined
        # result of joinFactors(allFactors) are actual CPT of given list of Factors after joining
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        if self.total() != 1:
            self.normalize()
        # sample by assigning interval based on distribution and then picking ranodm number
        # for example, P(R) = 0.2, P(G) = 0.4, P(B) = 0.4
        #  0 - 0.2 - 0.6 - 1.0
        #    R     G     B
        # when randomChoice is 0.4, it will mean P(G) which has range 0.2 to 0.6 is randomly picked
        randomChoice = random.random()
        items = sorted(self.items())
        probabilityDistributionList = [i[1] for i in items]
        cumulativeDistribution = probabilityDistributionList[0]
        itemIndex = 0

        while randomChoice > cumulativeDistribution:
            itemIndex += 1
            cumulativeDistribution += probabilityDistributionList[itemIndex]
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        # when observation is None and ghost is in jail position, return 1
        if noisyDistance == None and ghostPosition == jailPosition:
            return 1.0
        # if observation is None (irrespective of ghost being in jail) or ghost is in jail, return 0
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # self.beliefs is initial probability distribution of where ghosts are
        # given observation (noisy reading of distance to the ghost), use getObservationProb method to get 
        # observation of P(noisyDistance | trueDistance) and update self.beliefs
        pacmanPosition = gameState.getPacmanPosition()
        ...
        for ghostPosition in self.allPositions:
            ...
            discreteDistribution[ghostPosition] = probability * self.beliefs[ghostPosition]
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        # instead of relying on observation of noisy sensor, rely on possible ghost movement
        # using getPositionDistribution, we have { (coordinateTuple) : probability } of newGhostMovement
        # from currentProbability, update the probability for each possible newGhostPosition

        # since ghost tends to randomly move to south, the probability at each location
        # will tend to be higher at the south as we are updating each timestamp
        discreteDistribution = DiscreteDistribution()

        for ghostPosition in self.allPositions:
            newGhostDistribution = self.getPositionDistribution(gameState, ghostPosition)
            currentProbability = self.beliefs[ghostPosition]
            for newGhostPosition in newGhostDistribution:
                discreteDistribution[newGhostPosition] += currentProbability * newGhostDistribution[newGhostPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        """
        self.particles = []
        availableParticles = self.numParticles
        n = len(self.legalPositions)

        while availableParticles > 0:
            self.particles.append(self.legalPositions[availableParticles % n])
    ...
    def getBeliefDistribution(self):
        ...
        """
        discreteDistibution = DiscreteDistribution()

        for particle in self.particles:
            discreteDistibution[particle] += 1

        discreteDistibution.normalize()

        return discreteDistibution
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # initially with uniformly distributed particles, get observation and weight the particles
        for particlePosition in self.particles:
            particleProbability = self.getObservationProb(observation, pacmanPosition, particlePosition, jailPosition)
            discreteDistribution[particlePosition] += particleProbability
        
        # handling edge case where all weight of particles are zero
        if discreteDistribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            # update self.beliefs based on normalized distribution
            discreteDistribution.normalize()
            self.beliefs = discreteDistribution
            # after update, re-sample the particles by re-assigning all elements in self.particles
            for i in range(self.numParticles):
                # this sampling is based on weighted distribution above
                particleSample = discreteDistribution.sample()
                self.particles[i] = particleSample
    ...
    def elapseTime(self, gameState):
        ...
        gameState.
        """
        # since there are multiple particles in area of high probability
        # maintain cache of positionDistribution since getPositionDistribution is expensive
        particlePositionDistributionCache = {}

        for i,particle in enumerate(self.particles):
            if particle in particlePositionDistributionCache:
                self.particles[i] = particlePositionDistributionCache[particle].sample()
            else:
                # for each particle, get position distribution of new ghosts given current particle position
                # given the particle, this is weighting of each particles
                newGhostDistribution = self.getPositionDistribution(gameState, particle)
                newGhostDistribution.normalize()
                particlePositionDistributionCache[particle] = newGhostDistribution
                # from this newGhostDistribution, re-sample
                self.particles[i] = newGhostDistribution.sample()
